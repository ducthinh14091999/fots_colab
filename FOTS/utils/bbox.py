...
class Toolbox:
    ...
    @staticmethod
    def predict(im_fn, model, with_img=True, output_dir=None, with_gpu=False):
        ...
        if output_dir:
            ...
            plt.imshow(score.cpu().numpy()[0][0])
            plt.title('score map')
            plt.show()
            plt.imshow(geometry.cpu().numpy()[0][0])
            plt.title('geo up')
            # cv2.imshow('score',score.cpu().numpy())
            plt.show()
            # plt.imshow(geometry.cpu().numpy()[0][1])
            # plt.title('geo down')
            # # cv2.imshow('score',score.cpu().numpy())
            # plt.show()
            # plt.imshow(geometry.cpu().numpy()[0][2])
            # # cv2.imshow('score',score.cpu().numpy())
            # plt.title('geo right')
            # plt.show()
            # plt.imshow(geometry.cpu().numpy()[0][3])
            # plt.title('geo left')
            # # cv2.imshow('score',score.cpu().numpy())
            # plt.show()
            plt.imshow(im)
